...
#[derive(Clone)]
pub struct Client(mpsc::Sender<OutgoingQuery>);
...
impl Client {
    ...
    where
        Q: Into<QueryMsg>,
        R: TryFrom<ResponseMsg, Error = Error>,
    {
        ...
        self.0
            ...
            .await?;
        rx.await?.and_then(TryFrom::try_from)
    }
}

/// Server for receiving incoming queries from different nodes.
pub struct Server(mpsc::Receiver<IncomingQuery>);
...
pub struct Runner {
    ...
    outgoing_queries_source: mpsc::Receiver<OutgoingQuery>,
...
impl Runner {
    pub async fn run(mut self) -> Result<(), Error> {
        loop {
            ...
            select! {
                biased;
                outgoing = self.outgoing_queries_source.recv() => {
...
pub fn setup_routing(
    ...
    // We want to have backpressure on the app layer, hence the unit-sized channel
    let (outgoing_queries_sink, outgoing_queries_source) = mpsc::channel(1);

    // Ideally, we do not want to wait when sending our output to either layer, and we fail fast if we can't forward an incoming
    // query to the app layer. Hence the higher-capacity queue below.
    let (incoming_queries_sink, incoming_queries_source) = mpsc::channel(64);
...
impl<Q, R> AsRef<Q> for IncomingGenericQuery<Q, R> {
    fn as_ref(&self) -> &Q {
        &self.query
    }
}
...
type ResponseSender = oneshot::Sender<Result<ResponseMsg, Error>>;
...
mod details {
    ...
    pub(super) struct QueryManager {
        ...
        outgoing_msgs_sink: mpsc::Sender<(Message, SocketAddr)>,
        incoming_queries_sink: mpsc::Sender<IncomingQuery>,
    }
    ...
    impl QueryManager {
        ...
        pub(super) async fn handle_one_outgoing(
            ...
            let send_result = self.outgoing_msgs_sink.send((msg, query.destination_addr)).await;
        ...
        pub(super) async fn handle_one_incoming(
            ...
            if let MessageData::Query(request) = msg.data {
                // don't await a queue slot here, to prevent DoS
                let queue_slot = self.incoming_queries_sink.try_reserve()?;

                let response_sink = self.outgoing_msgs_sink.clone().reserve_owned().await?;
                let incoming_query =
                    convert_incoming_query(request, msg.transaction_id, response_sink, &src_addr);

                queue_slot.send(incoming_query);
                ...
                .and_then(|&tid| self.outstanding_requests.remove(&u16::from_be_bytes(tid)))
            {
                ...
                log::error!(
                    "Unhandled {} message",
                    if matches!(msg.data, MessageData::Response(_)) {
                        "response"
                    } else {
                        "error"
                    }
...
mod tests {
    ...
    fn test_incoming_ping_success() {
        ...
        assert_eq!(incoming_ping.as_ref().id, [1u8; 20].into());
    ...
    fn test_incoming_find_node_success() {
        ...
        assert_eq!(incoming_find_node.as_ref().id, [1u8; 20].into());
        assert_eq!(incoming_find_node.as_ref().target, [2u8; 20].into());
    ...
    fn test_incoming_get_peers_success() {
        ...
        assert_eq!(incoming_get_peers.as_ref().id, [1u8; 20].into());
        assert_eq!(incoming_get_peers.as_ref().info_hash, [2u8; 20].into());
    ...
    fn test_incoming_announce_peer_success() {
        ...
        assert_eq!(incoming_announce_peer.as_ref().id, [1u8; 20].into());
        assert_eq!(incoming_announce_peer.as_ref().info_hash, [2u8; 20].into());
        assert_eq!(incoming_announce_peer.as_ref().port, Some(1234));
        assert_eq!(incoming_announce_peer.as_ref().token, vec![3u8; 2]);
    ...
    fn test_concurrent_incoming_queries_out_of_order() {
        ...
        assert_eq!(incoming_ping.as_ref().id, [3u8; 20].into());
