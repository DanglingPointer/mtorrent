...
fn get_blocking_gateway(gw: &AsyncGateway) -> Gateway {
    Gateway {
        addr: gw.addr,
...
impl Drop for PortOpener {
    fn drop(&mut self) {
        let blocking_gw = get_blocking_gateway(&self.gateway);
        match blocking_gw.remove_port(self.proto, self.external_addr.port()) {
            Ok(()) => log::info!(
