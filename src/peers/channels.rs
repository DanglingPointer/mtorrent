...
impl<S: futures::AsyncReadExt + Unpin> IngressStream<S> {
    async fn read_one_message(&mut self) -> io::Result<()> {
        ...
        async fn forward_msg<M: TryFrom<PeerMessage> + fmt::Display>(
            msg: M,
        ...
        let received: PeerMessage = select! {
            msg_res = PeerMessage::read_from(&mut self.socket).fuse() => {
                msg_res
            }
    ...
    async fn write_one_message(&mut self) -> io::Result<()> {
        ...
        select! {
            rx_msg = self.rx_outbound.next().fuse() => {
                ...
                process_msg(msg, &mut self.rx_outbound, &mut self.socket, &self.remote_ip).await?;
            }
            tx_msg = self.tx_outbound.next().fuse() => {
                let msg = tx_msg.ok_or(io::Error::from(io::ErrorKind::Other))?;
                process_msg(msg, &mut self.tx_outbound, &mut self.socket, &self.remote_ip).await?;
            }
            ping_msg = delayed(PeerMessage::KeepAlive, ping_period).fuse() => {
                debug!("{} <= KeepAlive", self.remote_ip);
                ping_msg.write_to(&mut self.socket).await?;
            }
        };
        Ok(())
    }
...
#[cfg(test)]
async fn delayed<R>(what: R, by: Duration) -> R {
    let should_fire = tests::ELAPSED_TIME.with(|elapsed| {
        let prev_elapsed = elapsed.get();
        if prev_elapsed >= by {
            elapsed.set(prev_elapsed - by);
            true
        } else {
            false
        }
...
mod tests {
    ...
    thread_local! {
        pub static ELAPSED_TIME: Cell<Duration> = Cell::new(Duration::default());
    }
    ...
    fn test_write_downloader_message() {
        ...
        let run_fut = async {
            ...
            assert!(!runner.sender.socket.get_ref().get_ref().is_empty());

            runner.sender.socket.seek(SeekFrom::Start(0)).await.unwrap();
            let sent_message = PeerMessage::read_from(&mut runner.sender.socket)
                .await
                .unwrap();
    ...
    fn test_send_keepalive_every_30s() {
        ...
        ELAPSED_TIME.with(|elapsed| elapsed.set(Duration::from_secs(60)));
        ...
        assert_eq!(
            &two_keepalives,
            runner.borrow().sender.socket.get_ref().get_ref()
        );
    }
    ...
    fn test_receiver_times_out_after_2_min() {
        ...
        ELAPSED_TIME.with(|elapsed| elapsed.set(Duration::from_secs(120)));
    ...
    fn test_channel_send_receive_timeout() {
        ...
        ELAPSED_TIME.with(|elapsed| elapsed.set(Duration::from_secs(20)));
