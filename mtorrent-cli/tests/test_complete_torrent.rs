...
async fn listening_utp_peer<P: Peer>(
    ...
    let (connection_spawner, mut connect_reporter, io_driver) = utp::init(socket);
    let driver_handle = task::spawn_local(io_driver.run());

    macro_rules! accept_and_run {
        () => {{
            let (remote_addr, data) = time::timeout(sec!(10), connect_reporter.next())
                .await
...
async fn connecting_utp_peer<P: Peer>(
    ...
    let (connection_spawner, _, io_driver) = utp::init(socket);
    let driver_handle = task::spawn_local(io_driver.run());
