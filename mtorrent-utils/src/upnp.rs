...
impl PortMapper {
    ...
    async fn run_impl<G: GatewayFactory>(self, gateway_factory: G) -> Result<()> {
...
trait GatewayFactory {
    ...
    fn search(&self, opts: igd_next::SearchOptions) -> impl Future<Output = Result<Self::Gateway>>;
}

#[cfg_attr(test, mockall::automock)]
trait Gateway {
    fn get_external_ip(&self, timeout: Duration) -> impl Future<Output = Result<IpAddr>>;
...
impl<G: GatewayFactory> PortOpener<G> {
    ...
    async fn remove_mapping(&self) {
        ...
        match self.gateway.remove_port(proto, external_port, Self::GATEWAY_CALL_TIMEOUT).await {
            Ok(()) => {
...
mod tests {
    ...
    fn make_gateway_with_ip(ip: IpAddr) -> MockGateway {
        ...
        gw.expect_get_external_ip()
            .returning(move |_| async move { Ok(ip) }.boxed_local());
        gw
    }
    ...
    async fn test_creates_mapping_with_any_port() {
        ...
        let factory = make_factory_returning(move || {
            ...
            gw.expect_add_any_port()
                ...
                .returning(move |_, _, _, _, _| async move { Ok(assigned_port) }.boxed_local());
            gw.expect_remove_port()
                .once()
    ...
    async fn test_search_failure_is_reported_via_handle() {
        ...
        factory.expect_search().once().returning(|_opts| {
            async { Err(igd_next::SearchError::InvalidResponse.into()) }.boxed_local()
        });
    ...
    async fn test_mapping_is_renewed_periodically() {
        ...
        let factory = make_factory_returning(move || {
            ...
            gw.expect_add_port()
                ...
                .returning(|_, _, _, _, _, _| async { Ok(()) }.boxed_local());
            gw.expect_remove_port().once().returning(|_, _, _| async { Ok(()) }.boxed_local());
            gw
        });
