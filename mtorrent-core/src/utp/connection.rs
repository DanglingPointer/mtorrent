        ...
        loop {
            select! {
                ...
                read_result = self.receiver.read_buf(&mut send_buffer), if tx_allowed!() => {
                    match read_result {
                        Err(e) => {
                            log::info!("Egress processor exiting: pipe failure ({e})");
                            return Ok(());
                        }
                        Ok(0) if Limit::limit(&send_buffer) > 0 => {
                            log::info!("Egress processor exiting: pipe closed");
                            return Ok(());
                        }
                        Ok(_bytes_read) => {}
                    }
                    ...
                    match header.type_ver() {
                        ...
                        TypeVer::Data => {
                            if let Err(e) = self.sender.write_all_buf(&mut packet).await {
                                log::debug!("Ingress processor exiting: pipe closed ({e})");
                                return Ok(());
                            }
                            with!(|state| state.process_header(&header));
                ...
                Err(e) => match e {
                    ...
                    ValidationError::Duplicate => {
                        if log_enabled!(log::Level::Debug) {
                            log::debug!(
                                "Received duplicate {:?} packet, seq_nr={}, ack_nr={}",
                                header.type_ver(),
                                header.seq_nr(),
                                header.ack_nr()
                            );
                    ...
                    e @ ValidationError::OutOfOrder { .. } => {
                        if log_enabled!(log::Level::Debug) {
                            log::debug!("Received out-of-order packet: {e}");
                        }
                    }
                },
            }
...
impl Connection {
    ...
    pub async fn outbound(
        ...
        let mut state = ConnectionState::new_outbound();
        ...
        egress.send(buffer.freeze()).await?;

        // receive STATE
        let mut packet = ingress.next().await.ok_or(io::ErrorKind::BrokenPipe)?;
        let header = Header::decode_from(&mut packet)?;
        match header.type_ver() {
            TypeVer::State => {
